...
{
    /// <summary>
    /// Interaction logic for ucBooksType.xaml
    /// </summary>
    public partial class UcBooksType
    {
        ...
        {
            _bLoaded = true;
            foreach (string item in new BookServices().GetTypesName())
            {
        ...
        private void addCustomeType_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(addlink.Link) == false)
            {
                ...
                GenreServices.GetGenre(newtype, EntityType.Books);
